...
async fn list_records(
    ...
    let mut sort_clause = "ORDER BY created_at DESC".to_string();
    // TODO: Phase 2 - Implement View filters -> dynamic SQL WHERE clause
    if let Some(view_id) = query.view_id {
        // For now, just logging that we "would" load the view
        // let _view = state.metadata.get_view(tenant_id, view_id).await...
        // view.sort -> parse -> update sort_clause
    }

    // 4. Query
    // We strictly use `entity_records`.
...
async fn create_record(
    ...
    // 1. Resolve Metadata
    // Note: get_fields_by_entity_name internally calls get_entity_type -> ensures validation
...
async fn get_record(
    State(state): State<Arc<AppState>>,
    Path((_entity_code, id)): Path<(String, Uuid)>, // We don't strictly *need* entity_code for fetch by ID, but good for validation
    axum::Extension(tenant): axum::Extension<ResolvedTenant>,
) -> impl IntoResponse {
    let mut conn = match get_rls_connection(&state.pool, tenant.id).await {
        Ok(c) => c,
...
async fn lookup_entity(
    ...
    // Search query
    // Note: We search in the derived display string OR explicit fields
    let sql = format!(
        r#"
        SELECT id, {} as label
        FROM entity_records 
        ...
        Ok(results) => {
            let data: Vec<LookupResult> = results.iter().map(|row| {
                 LookupResult {
                     ...
                     sub_label: None, // Could use for secondary info
                 }
            }).collect();
            Json(data).into_response()
        },
...
fn validate_and_process_payload(
    ...
    for field in fields {
        // Skip ID fields or system fields if present in definitions? 
        // Assuming definitions only contain user-editable fields.
...
fn validate_field_type(field: &FieldDef, value: &Value) -> Result<(), String> {
    match field.field_type {
        ...
        FieldType::Number | FieldType::Money | FieldType::Score => {
             // JSON numbers can be i64 or f64.
             if !value.is_number() {
                 return Err(format!("Field '{}' must be a number", field.label));
             }
             if let Some(val) = &field.validation {
                 let n = value.as_f64().unwrap(); // Safe?
        ...
        FieldType::Select { .. } | FieldType::MultiSelect { .. } => {
            // Check if value is in options? Phase 2.
            // For now just ensure it's string or array of strings.
        },
        FieldType::Date => {
        ...
        FieldType::Link { .. } => {
             // Links are UUIDs usually sent as strings
